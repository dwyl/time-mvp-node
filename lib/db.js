  ...
  PG_CLIENT.query(SELECT, function(err, result) {
    if(err || result.rows.length === 0) {
      ...
      PG_CLIENT.query(INSERT, function(err, result) {
        // console.log(err, result)
        console.log('L115 ---> person INSERTed:', req.json.email_address);
        send_verification_email(req, verify_token); // fire & forget!
...
function send_verification_email(req, verify_token) {
  var person = {
  ...
  email(person, function(err, res) {
    console.log(err, res); // we aren't handling the error ... YET!
  });
}
